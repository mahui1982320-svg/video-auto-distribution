<template>
  ...
  <el-container v-else class="shell">
    <el-aside width="232px" class="sidebar">
      ...
      <el-menu :default-active="page" @select="selectPage">
        ...
        <el-menu-item v-if="isAdmin" index="publish"><el-icon><Promotion /></el-icon>集中发布</el-menu-item>
        ...
        <el-menu-item v-if="isAdmin" index="accounts"><el-icon><Connection /></el-icon>账号资产</el-menu-item>
      ...
      <el-main>
        ...
        <template v-else-if="page === 'publish'">
          <section class="panel publish-panel">
            <div class="section-head"><div><span class="eyebrow">CENTRAL DELIVERY</span><h3>集中创建发布任务</h3><p>先选择同事，再选择其已登录的平台账号；所有账号扫码和切换均由当前管理员统一完成。</p></div></div>
            <el-form label-position="top" class="publish-form">
              ...
              <el-form-item label="发布对象（必选）"><el-select v-model="publish.owner_user_id" placeholder="先选择要发布的同事" filterable><el-option v-for="u in teamMembers" :key="u.id" :label="`${u.display_name}（${u.username}）`" :value="u.id" /></el-select><small class="field-tip">该选择决定下方可用的平台账号，防止内容误发到其他同事账号。</small></el-form-item>
              ...
              <el-form-item label="平台账号（必选）"><div class="account-heading"><span>{{ publish.owner_user_id ? '请选择该同事在上述平台对应的已登录账号' : '请先选择发布对象' }}</span><el-button type="primary" plain @click="openAccountFromPublish" :disabled="!publish.owner_user_id"><el-icon><Plus /></el-icon>添加该同事的平台账号</el-button></div><el-checkbox-group v-if="filteredPublishAccounts.length" v-model="publish.account_ids" class="account-picker"><el-checkbox v-for="a in filteredPublishAccounts" :key="a.id" :value="a.id" border><span class="platform-dot" :class="a.platform"></span>{{ platformName(a.platform) }} · {{ a.display_name }} <el-tag size="small" :type="a.status === 'ready' ? 'success' : 'warning'">{{ statusName(a.status) }}</el-tag></el-checkbox></el-checkbox-group><el-empty v-else :description="publish.owner_user_id ? (publish.platforms.length ? '该同事在所选平台暂无账号，请点击添加' : '请先选择发布平台') : '请先选择发布对象'" :image-size="72" /></el-form-item>
        ...
        <template v-else-if="page === 'accounts'">
          <section class="panel"><div class="section-head"><div><span class="eyebrow">CHANNEL ASSETS</span><h3>集中账号资产</h3><p>编导统一扫码和维护登录凭证；每个账号明确归属一位同事，发布时按同事切换。</p></div><el-button type="primary" @click="openAccountManager"><el-icon><Plus /></el-icon>添加账号</el-button></div>
            <el-table :data="accounts"><el-table-column label="所属同事" min-width="180"><template #default="s"><el-select v-model="s.row.owner_user_id" placeholder="请选择" size="small" @change="updateAccountOwner(s.row)"><el-option v-for="u in teamMembers" :key="u.id" :label="`${u.display_name}（${u.username}）`" :value="u.id" /></el-select></template></el-table-column><el-table-column label="平台" width="130"><template #default="s"><span class="platform-dot" :class="s.row.platform"></span>{{ platformName(s.row.platform) }}</template></el-table-column><el-table-column prop="display_name" label="账号备注名" /><el-table-column label="登录状态" width="130"><template #default="s"><el-tag :type="s.row.status === 'ready' ? 'success' : 'warning'">{{ statusName(s.row.status) }}</el-tag></template></el-table-column><el-table-column label="操作" width="220"><template #default="s"><el-button size="small" @click="startAccountLogin(s.row)">管理员扫码</el-button><el-button size="small" type="danger" plain @click="deleteAccount(s.row)">删除</el-button></template></el-table-column></el-table>
          </section>
        </template>
  ...
  <el-dialog v-model="accountDialog" title="添加并登录平台账号" width="500px" :close-on-click-modal="false"><el-alert title="操作步骤：选择所属同事 → 选择平台 → 填写备注名 → 由管理员使用平台 App 扫码登录" type="info" :closable="false" show-icon /><el-form label-position="top" class="account-form"><el-form-item label="第 1 步：账号所属同事"><el-select v-model="accountForm.owner_user_id" placeholder="请选择该账号属于谁"><el-option v-for="u in teamMembers" :key="u.id" :label="`${u.display_name}（${u.username}）`" :value="u.id" /></el-select></el-form-item><el-form-item label="第 2 步：选择平台"><el-select v-model="accountForm.platform"><el-option v-for="p in platforms" :key="p.value" :label="p.label" :value="p.value" /></el-select></el-form-item><el-form-item label="第 3 步：填写账号备注名"><el-input v-model="accountForm.display_name" placeholder="例如：小王的抖音主账号" /><small class="field-tip">这里只是系统内显示的名称，不需要填写平台用户名或密码。</small></el-form-item></el-form><template #footer><el-button @click="accountDialog=false">取消</el-button><el-button type="primary" :loading="accountCreating" @click="createAccount">创建并扫码登录</el-button></template></el-dialog>
...
const publish = reactive({ material_id: null, owner_user_id: null, platforms: [], account_ids: [], title: '', description: '', tags: '', schedule_at: '' })
const accountDialog = ref(false), accountCreating = ref(false), accountForm = reactive({ owner_user_id: null, platform: 'douyin', display_name: '' })
...
const availablePlatforms = computed(() => platforms.filter(p => accounts.value.some(a => a.platform === p.value)))
const teamMembers = computed(() => users.value.filter(x => x.active))
const filteredPublishAccounts = computed(() => accounts.value.filter(a => publish.owner_user_id === a.owner_user_id && publish.platforms.includes(a.platform)))
const pageTitle = computed(() => ({dashboard:'工作台',publish:'集中发布',jobs:'发布记录',materials:'素材库',accounts:'账号资产',users:'员工管理',audit:'操作日志'}[page.value]))
...
async function createJob(){if(!publish.material_id)return ElMessage.warning('请选择视频素材');if(!publish.owner_user_id)return ElMessage.warning('请先选择要发布的同事');if(!publish.platforms.length)return ElMessage.warning('请选择至少一个发布平台');if(!publish.account_ids.length)return ElMessage.warning('请选择至少一个平台账号');const uncovered=publish.platforms.filter(p=>!publish.account_ids.some(id=>accounts.value.find(a=>a.id===id)?.platform===p));if(uncovered.length)return ElMessage.warning(`请为${uncovered.map(platformName).join('、')}选择发布账号`);loading.value=true;try{await api('/jobs',{method:'POST',body:JSON.stringify(publish)});ElMessage.success('发布任务已创建，正在提交至平台');Object.assign(publish,{material_id:null,owner_user_id:null,platforms:[],account_ids:[],title:'',description:'',tags:'',schedule_at:''});page.value='jobs';await refresh()}catch(e){ElMessage.error(e.message)}finally{loading.value=false}}
function openAccountFromPublish(){accountDialogSource.value='publish';Object.assign(accountForm,{owner_user_id:publish.owner_user_id,platform:publish.platforms[0]||'douyin',display_name:''});accountDialog.value=true}
function openAccountManager(){accountDialogSource.value='accounts';Object.assign(accountForm,{owner_user_id:null,platform:'douyin',display_name:''});accountDialog.value=true}
async function createAccount(){if(!accountForm.owner_user_id)return ElMessage.warning('请选择账号所属同事');if(!accountForm.display_name.trim())return ElMessage.warning('请填写账号备注名');accountCreating.value=true;try{const created=await api('/accounts',{method:'POST',body:JSON.stringify(accountForm)});const selectedPlatform=accountForm.platform;accountDialog.value=false;Object.assign(accountForm,{owner_user_id:null,platform:'douyin',display_name:''});await refresh();if(accountDialogSource.value==='publish'){if(!publish.platforms.includes(selectedPlatform))publish.platforms.push(selectedPlatform);if(!publish.account_ids.includes(created.id))publish.account_ids.push(created.id)}const row=accounts.value.find(a=>a.id===created.id);ElMessage.success('账号已创建，请完成管理员扫码登录');if(row)await startAccountLogin(row)}catch(e){ElMessage.error(e.message)}finally{accountCreating.value=false}}
...
async function updateAccountOwner(row){try{await api(`/accounts/${row.id}/owner`,{method:'PATCH',body:JSON.stringify({owner_user_id:row.owner_user_id})});ElMessage.success('账号归属已更新');await refresh()}catch(e){ElMessage.error(e.message);await refresh()}}
...
watch(()=>publish.owner_user_id, ()=>{publish.account_ids=[]})
